...
char html_page[] = "<!DOCTYPE HTML><html>\n"                       // documento com padrão HTML5
                   "<head>\n"                       // Inicia o cabeçalho
                   "  <meta charset=\"UTF-8\">"
                   "  <title>ESP-IDF Server - Monitoramento do Consumo de Energia</title>\n"        // Título da página
                   ...
                   "  <div class=\"topnav\">\n"
                   "    <h3>ESP-IDF Server - Monitoramento do Consumo de Energia</h3>\n"     // Exibe o título da página no cabeçalho
                   "  </div>\n"
...
void verificar_alerta() {
    ...
    if (alerta_ativo) {
        ...
        if (tempo_decorrido >= pdMS_TO_TICKS(30000)) { // 30 segundos
...
void verifica_tensao(float voltage_rms) {
    ...
    if (voltage_rms > 242.0) {  // Verifica se a tensão está acima de 242V (1.1 PU)
        sobretensao = 1;
    }
